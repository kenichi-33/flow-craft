...
async function bootstrap() {
  ...
  try {
    ...
    logger.log(
      `Created App ${appId} and Task ${taskId} with initial data: ${JSON.stringify(application.inputData)}`,
    );
    ...
    logger.log(
      `Updated App inputData: ${JSON.stringify(updatedApp?.inputData)}`,
    );

    if (
      updatedApp &&
      updatedApp.inputData &&
      (updatedApp.inputData as any).initial === 'updated'
    ) {
