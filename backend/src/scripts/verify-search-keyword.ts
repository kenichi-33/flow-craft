...
async function main() {
  ...
  try {
    ...
    if (!selectField) {
      console.warn(
        '⚠️ No Select/Radio/Checkbox field found with options. Skipping Label search test part.',
      );
    } else {
      console.log(
        `ℹ️ Found option field '${selectField}' with label '${targetOption.label}' (value: ${targetOption.value})`,
      );
    }
    ...
    console.log(
      `✅ App Created: ${application.id} (No. ${application.applicationNumber})`,
    );
    ...
    if (selectField) {
      process.stdout.write(
        `Test 1: Search by Label '${targetOption.label}' ... `,
      );
    ...
    process.stdout.write(
      `Test 3: Search by Applicant Name '${fakeLastName}' ... `,
    );
    ...
    process.stdout.write(
      `Test 4: Search by App Number '${application.applicationNumber}' ... `,
    );
    const resNum = await searchService.search({
      keyword: String(application.applicationNumber),
    });
