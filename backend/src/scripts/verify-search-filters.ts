...
import { SearchQueryDto, SearchOperator } from '../modules/search/dto/search-application.dto';
...
async function main() {
  ...
  try {
    ...
    if (!appDef.formDefinitionId) throw new Error('App definition has no form definition.');

    // Get the form definition to find real field IDs
    const formDef = await prisma.formDefinition.findUnique({ where: { id: appDef.formDefinitionId } });
    ...
    for (const [key, conf] of Object.entries(props)) {
        ...
        if (c.type === 'number') amountField = key as string;
        if (['text', 'textarea', 'email', 'url'].includes(c.type)) reasonField = key as string;
    ...
    console.log(`Using fields - Amount: ${amountField}, Reason: ${reasonField}`);
    ...
    console.log(`Creating application with inputData: ${JSON.stringify(inputData)}...`);
    ...
    console.log(!found4 ? '✅ SUCCESS (Not found)' : '❌ FAILURE (Found unexpectedly)');
