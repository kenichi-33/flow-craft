...
export class MasterConnectorsService {
  ...
  async findAll(user: any) {
    ...
    return Promise.all(connectors.map(async (c) => {
        ...
        if (c.createdBy) {
            createdByInfo = await this.usersService.getUserSnapshotByUsername(c.createdBy);
        }
        if (c.updatedBy) {
            updatedByInfo = await this.usersService.getUserSnapshotByUsername(c.updatedBy);
        }
        return {
            ...c,
            createdByInfo,
  ...
  async findOne(id: string) {
    ...
    if (connector.createdBy) {
        connector.createdByInfo = await this.usersService.getUserSnapshotByUsername(connector.createdBy);
    }
    if (connector.updatedBy) {
        connector.updatedByInfo = await this.usersService.getUserSnapshotByUsername(connector.updatedBy);
    }
