...
export class MasterConnectorsController {
  constructor(
    private readonly masterConnectorsService: MasterConnectorsService,
  ) {}

  @Post()
  create(
    @Body() createMasterConnectorDto: CreateMasterConnectorDto,
  ...
  @Post('test')
  test(
    @Body() body: { config: any; mapping: any; query: string; type?: string },
  ) {
    return this.masterConnectorsService.test(
      body.config,
      body.mapping,
      body.query,
      body.type,
  ...
  uploadCsv(
    ...
    if (!file) {
      ...
      throw new BadRequestException(
        'File is required and must be named "file"',
      );
    }
    console.log(
  ...
  update(
    @Param('id') id: string,
    @Body() updateMasterConnectorDto: UpdateMasterConnectorDto,
    @CurrentUser() user: any,
  ) {
    return this.masterConnectorsService.update(
      id,
      updateMasterConnectorDto,
      user,
    );
  }
