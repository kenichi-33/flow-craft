...
export class StorageService implements OnModuleInit {
    ...
    constructor(
        ...
        const internalEndpoint = this.configService.get('S3_ENDPOINT') || 'http://minio:9000';
        ...
        const publicEndpoint = this.configService.get('S3_PUBLIC_URL') || 'http://localhost:9000';
    ...
    async createPresignedUploadUrl(
        ...
        if (size > this.MAX_FILE_SIZE) {
            throw new BadRequestException(`ファイルサイズが上限(${this.MAX_FILE_SIZE / 1024 / 1024}MB)を超えています`);
        }
     ...
     * 署名付きダウンロードURLを生成
     */
    async createPresignedDownloadUrl(fileId: string): Promise<{ url: string; filename: string }> {
        const file = await this.prisma.file.findUnique({ where: { id: fileId } });
