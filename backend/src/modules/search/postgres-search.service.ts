...
export class PostgresSearchService implements ISearchService {
  ...
  async search(dto: SearchQueryDto): Promise<SearchResult<Application>> {
    ...
    if (applicationDefinitionId) where.applicationDefinitionId = applicationDefinitionId;
    ...
    if (sort) {
        if (['createdAt', 'updatedAt', 'applicationNumber', 'status', 'title'].includes(sort.field)) {
  ...
  async indexApplication(app: Application): Promise<void> {
    ...
    if (!fullApp || !fullApp.formDefinition) {
        this.logger.warn(`App ${app.id} or FormDefinition not found for indexing`);
        return;
    }
