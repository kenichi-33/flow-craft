...
@Injectable()
export class PgBossQueueAdapter implements IQueueAdapter, ISchedulerAdapter, OnModuleInit, OnModuleDestroy {
  private boss: PgBoss;
  ...
  async enqueue(topic: string, payload: any, options?: { delay?: number }): Promise<void> {
    try {
      ...
      this.logger.debug(`Job enqueued to ${topic} with options ${JSON.stringify(sendOptions)}: ${JSON.stringify(payload)}`);
    } catch (error) {
  ...
  async schedule(name: string, cron: string, topic: string, payload: any): Promise<void> {
    try {
  ...
  async unschedule(name: string): Promise<void> {
    try {
        ...
        const schedules = await this.configService.get('DATABASE_URL') ? [] : []; // Placeholder logic
        this.logger.warn(`Unschedule requested for ${name} but ID lookup not implemented fully in adapter yet.`);
