      ...
      uniqueIds.map(async (id) => {
        try {
          ...
          return {
            ...
            displayName:
              `${snapshot.lastName || ''} ${snapshot.firstName || ''}`.trim() ||
              snapshot.username,
      ...
      for (const a of assignments) {
        ...
        if (a.startsWith('group:')) {
          ...
          if (userResponse.data.length > 0) {
            ...
            const targetGroup =
              await this.usersService.findGroupByIdentifier(groupIdentifier);
            ...
            if (targetGroup) {
              ...
              // パスの一致（またはサブグループ）を確認
              const isMatch = groupsResponse.data.some(
                (g) =>
              ...
              console.warn(
                `[Users] Group with deptCode '${groupIdentifier}' not found`,
              );
              ...
              if (team) {
                ...
                const isMember = team.members.some(
                  (m) => m.memberType === 'user' && m.memberId === username, // TODO: Check by ID or Username? Schema says memberId is string. Service uses username for user type.
                );
                ...
                const userGroups =
                  await this.usersService.getUserGroupsWithDeptCode(username);
                const userDeptCodes = userGroups
                  .map((g) => g.deptCode)
                  .filter(Boolean);

                const isDeptMember = team.members.some(
                  (m) =>
                    m.memberType === 'department' &&
                    userDeptCodes.includes(m.memberId),
                );
