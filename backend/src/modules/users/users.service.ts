...
export class UsersService {
  ...
  constructor(
    ...
    this.keycloakUrl =
      this.configService.get('KEYCLOAK_URL') || 'http://localhost:8081';
  ...
  public async getGroups(
    rootPath?: string,
  ): Promise<{ id: string; name: string; path: string; deptCode?: string }[]> {
  ...
  public async getAllDepartments(): Promise<
    { id: string; name: string; path: string; deptCode?: string }[]
  > {
  ...
  public async findGroupByIdentifier(
    identifier: string,
  ): Promise<{ path: string; name: string; deptCode?: string } | null> {
    const depts = await this.getAllDepartments();
  ...
  async getGroupMembersByIdentifier(
    identifier: string,
    ...
    try {
      ...
      if (team) {
        ...
        for (const m of team.members) {
          if (m.memberType === 'user') {
            ...
            const groupSnaps = await this.getGroupMembersByIdentifier(
              m.memberId,
            );
      ...
      console.warn(
        `[UsersService] Failed to lookup team members for ${identifier}`,
        e,
      );
    }
  ...
  async resolveAssignedToSnapshot(assignedTo: string): Promise<UserSnapshot> {
    ...
    if (assignedTo.startsWith('group:')) {
      ...
      try {
        ...
        console.warn(
          `[UsersService] Failed to lookup team for ${groupCodeOrPath}`,
          e,
        );
      }
   ...
   * グループ認可チェック用
   */
  async getUserGroupsWithDeptCode(
    userIdOrUsername: string,
  ): Promise<{ id: string; path: string; name: string; deptCode?: string }[]> {
    try {
      const token = await this.getAdminToken();
      ...
      console.error(
        `Failed to get groups for user ${userIdOrUsername}:`,
        error,
      );
      return [];
    }
