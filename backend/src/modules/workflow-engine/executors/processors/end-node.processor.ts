...
export class EndNodeProcessor implements INodeProcessor {
  ...
  async process(
    ...
    console.log(
      `[EndNodeProcessor] applicationId=${applicationId}, parentId=${parentId}, currentApp.status=${currentApp.status}`,
    );
    ...
    if (parentId) {
      ...
      if (parentApp) {
        ...
        // Check if all siblings are completed
          ...
          console.log(
            `[EndNodeProcessor] AI Chat Parent Check: parentId=${parentId}, currentAppId=${applicationId}, status=${status}`,
          );
          ...
          const allCompleted = siblings.every((app) => {
            ...
            if (app.id === applicationId) {
              return ['COMPLETED', 'APPROVED', 'REJECTED', 'CANCELED'].includes(
                status,
              );
            }
            return ['COMPLETED', 'APPROVED', 'REJECTED', 'CANCELED'].includes(
          ...
          if (allCompleted) {
            console.log(
              `[EndNodeProcessor] All siblings completed! parentApp.status=${parentApp.status}, parentNodeId=${parentNodeId}`,
            );
