...
export class ServiceTaskProcessor implements INodeProcessor {
    ...
    async process(context: NodeProcessorContext, tx: Prisma.TransactionClient): Promise<void> {
        ...
        if (existingTask) {
            this.logger.log(`Reusing existing task ${existingTask.id} for node ${nodeId} (Status: ${existingTask.status})`);
        ...
        await tx.approvalHistory.create({
            data: {
                applicationId,
                actorId: 'SYSTEM',
                action: node.type === 'sendEmail' ? 'SEND_EMAIL' : 
                        node.type === 'slack' ? 'SLACK_NOTIFY' : 
