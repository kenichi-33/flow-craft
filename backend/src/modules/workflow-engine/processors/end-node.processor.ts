...
export class EndNodeProcessor implements INodeProcessor {
    ...
    async process(context: NodeProcessorContext, tx: Prisma.TransactionClient): Promise<void> {
        ...
        if ((currentApp as any)?.parentId) {
            ...
            if (parentApp) {
                const flowNodes = (parentApp.flowNodes || parentApp.flowDefinition.nodes || []) as any[];
                ...
                if (subProcessNode && subProcessNode.type === 'subProcess') {
                    ...
                    let newParentInputData = { ...(parentApp.inputData as any) };
                    ...
                    if (config.outputMapping) {
                         ...
                         for (const [key, template] of Object.entries(mapping)) {
                             const value = this.helper.substituteVariables(template, substitutionContext);
                    ...
                    await this.helper.advanceToNextNode(parentId, 0, parentNodeId || undefined);
                }
            }
        }
    }
