...
export class SubProcessProcessor implements INodeProcessor {
    ...
    async process(context: NodeProcessorContext, tx: Prisma.TransactionClient): Promise<void> {
        ...
        if (!childAppDefId) {
            this.logger.error(`SubProcess Node ${nodeId} missing applicationDefinitionId`);
            throw new Error('SubProcess configuration error: applicationDefinitionId is missing');
        }

        this.logger.log(`Starting SubProcess for app ${applicationId} -> Def ${childAppDefId}`);

        // 1. Fetch Definition
        const appDef = await tx.applicationDefinition.findUnique({ // Use tx
        ...
        const flowNodes = publishedVersion?.flowNodes ?? appDef.flowDefinition?.nodes;
        const flowEdges = publishedVersion?.flowEdges ?? appDef.flowDefinition?.edges;
        const formSchema = publishedVersion?.formSchema ?? appDef.formDefinition?.schema;
        const startNode = (flowNodes as any[] || []).find(n => n.type === 'start');
        ...
        const applicantInfo = await this.helper.resolveAssignedToSnapshot('applicant'); // or reuse parent's applicantInfo via fetch?
