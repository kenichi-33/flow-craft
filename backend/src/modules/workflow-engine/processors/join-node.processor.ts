...
export class JoinGatewayProcessor implements INodeProcessor {
    ...
    async process(context: NodeProcessorContext, tx: Prisma.TransactionClient): Promise<void> {
        const { applicationId, nodeId, edges, inputData, fromNodeId, node } = context;
        ...
        if (joinType === 'ANY') {
            this.logger.log(`[JoinGateway] Node ${nodeId} (Type: ANY) processing arrival from ${fromNodeId}. Passing through.`);
        ...
        if (fromNodeId && incomingNodeIds.includes(fromNodeId)) {
            if (!myJoinState.arrivedFrom.includes(fromNodeId)) {
                ...
                this.logger.log(`[JoinGateway] Node ${nodeId} recorded arrival from ${fromNodeId}. Current: [${myJoinState.arrivedFrom.join(', ')}]`);
            }
        } else {
            this.logger.warn(`[JoinGateway] Arrived at ${nodeId} from unknown or untracked node: ${fromNodeId}`);
        }

        // 3. Check if ALL incoming paths have arrived
        const allArrived = incomingNodeIds.every((sourceId: string) => myJoinState.arrivedFrom.includes(sourceId));
        ...
        if (!allArrived) {
            this.logger.log(`[JoinGateway] Node ${nodeId} Waiting. Arrived: ${myJoinState.arrivedFrom.length}/${incomingNodeIds.length}`);
        ...
        this.logger.log(`[JoinGateway] Node ${nodeId} All paths arrived. Resetting state and Advancing.`);
