...
const EMAIL_TEMPLATES: Record<string, { subject: string; body: string }> = {
  approval_request: {
    subject:
      '【{{applicationDefinition.name}}】承認依頼: {{application.title}}',
    body: '申請「{{application.title}}」の承認依頼が届いています。\n\nアプリ: {{applicationDefinition.name}}\n申請者: {{applicant.name}}\nリンク: {{applicationUrl}}',
  },
  approval_remind: {
    subject:
      '【{{applicationDefinition.name}}】リマインド: 承認期限が迫っています',
...
export class EmailTaskHandler implements ITaskHandler {
  ...
  async execute(context: TaskContext): Promise<TaskResult> {
    ...
    try {
      ...
      const applicantUser = await this.helper.resolveAssignedToSnapshot(
        `user:${applicantId}`,
      );
      const applicantName = applicantUser
        ? `${applicantUser.lastName} ${applicantUser.firstName}`
        : applicantId;
      ...
      const toStr = this.helper.substituteVariables(
        config.to,
        substitutionData,
      );
      ...
      if (toList.length === 0) {
        ...
        this.logger.warn(
          `Email task ${taskId}: Resolved 0 recipients from "${toStr}"`,
        );
        // Should fail or skip?
      ...
      this.logger.log(
        `Sending email to ${toList.length} recipients: ${toList.join(', ')}`,
      );

      // Send to all
      await Promise.all(
        toList.map((email) => this.mailService.sendEmail(email, subject, body)),
      );
