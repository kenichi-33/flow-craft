...
export class AiCheckHandler implements ITaskHandler {
  ...
  async execute(context: TaskContext): Promise<TaskResult> {
    ...
    try {
      this.logger.log(`[DEBUG] AI Check Handler started for task ${taskId}, node ${nodeId}, targetSource ${nodeData.targetSource}`);
      ...
      if (checkResult.passed) {
        ...
        // Validation Failed
        const logs = [`Check Failed: ${checkResult.reasoning}`, `Feedback: ${checkResult.feedback}`];
        ...
        if (actionOnFail === 'error') {
          ...
          // REMAND (Default)
          this.logger.log(`AI Check failed, remanding application ${applicationId}`);
          ...
          if (remandTo === 'previous') {
             ...
             if (lastTask) {
                 ...
                 this.logger.log(`Remanding to previous step: ${remandTargetStepId} (Task: ${lastTask.id})`);
             } else {
                 this.logger.warn('No previous task found for remand, falling back to applicant');
             }
          }
          ...
          await this.processRemand(applicationId, taskId, checkResult.feedback, remandTargetStepId);
   ...
   */
  private async processRemand(applicationId: string, currentTaskId: string, feedback: string, targetStepId?: string) {
