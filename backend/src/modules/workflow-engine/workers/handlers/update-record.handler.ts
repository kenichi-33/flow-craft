...
export class UpdateRecordHandler implements ITaskHandler {
    ...
    async execute(context: TaskContext): Promise<TaskResult> {
        ...
        try {
            ...
            const updates: Array<{ key: string, value: string }> = config.updates || config.variables || [];
            ...
            for (const v of updates) {
                if (v.key) {
                    // Substitute value
                    const val = this.helper.substituteVariables(v.value, substitutionData);
                    outputData[v.key] = val;
                }
            }

            this.logger.log(`Updating record fields for app ${applicationId}: ${Object.keys(outputData).join(', ')}`);
