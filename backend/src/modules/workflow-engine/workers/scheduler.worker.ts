...
export class SchedulerWorker implements OnModuleInit {
    ...
    async onModuleInit() {
        await this.queueService.registerHandler('WORKFLOW_START', this.processJob.bind(this));
    ...
    async processJob(job: any): Promise<void> {
        ...
        this.logger.log(`Processing scheduled workflow start for AppDef: ${applicationDefinitionId}`);
        ...
        try {
            ...
            this.logger.log(`Successfully started workflow for AppDef: ${applicationDefinitionId}`);
        } catch (error) {
            this.logger.error(`Failed to start scheduled workflow: ${error.message}`, error.stack);
            throw error;
        }
    }
}
