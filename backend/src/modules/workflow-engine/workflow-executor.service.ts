...
export class WorkflowExecutorService implements OnModuleInit {
    ...
    async handleNodeProcessingJob(job: { applicationId: string, fromNodeId?: string, targetNodeId?: string }) {
        this.logger.log(`Processing workflow node for application ${job.applicationId} (target: ${job.targetNodeId || 'auto'}, from: ${job.fromNodeId || 'auto'})`);
     ...
     */
    private async processNode(applicationId: string, targetNodeId?: string, fromNodeId?: string) {
        ...
        if (!nextNodeId) {
            ...
            if (!sourceInfoId) {
                ...
                this.logger.warn(`No source node ID found (from: ${fromNodeId}, current: ${application.currentNodeId})`);
                return;
            }
        ...
        try {
            ...
            this.logger.error(`Error processing node ${nextNodeId} (${nextNode.type})`, e);
            throw e; 
        }
    }
}
