...
export class ApplicationDefinitionsService {
  ...
  async create(createDto: CreateApplicationDefinitionDto, user: AuthUser) {
    ...
    if (appDef.scheduleCron && appDef.status === AppDefStatus.ACTIVE) {
      await this.schedulerService.scheduleWorkflow(
        appDef.id,
        appDef.scheduleCron,
        { applicationDefinitionId: appDef.id, triggeredBy: 'schedule' },
      );
    }
  ...
  async findAll(options: FindAllOptions = {}, user?: AuthUser) {
    ...
    if (search) {
      ...
      if (where.OR) {
        ...
        delete where.OR;
        where.AND = [{ OR: accessOR }, { OR: searchCondition }];
      } else {
  ...
  async findOne(id: string) {
    ...
    if (!appDef) {
      throw new NotFoundException(
        `ApplicationDefinition with ID ${id} not found`,
      );
    }
    ...
    if (appDef.adminIds && appDef.adminIds.length > 0) {
      adminInfo = await Promise.all(
        appDef.adminIds.map(async (adminId: string) => {
          const isUuid =
          ...
          if (isUuid) {
            ...
            snapshot =
              await this.usersService.getUserSnapshotByUsername(adminId);
            snapshot.id = adminId;
          }
  ...
  async update(
    id: string,
    updateDto: UpdateApplicationDefinitionDto,
    username: string,
  ) {
    ...
    if (cronChanged || statusChanged) {
      ...
      if (updated.scheduleCron && isActive) {
        await this.schedulerService.scheduleWorkflow(id, updated.scheduleCron, {
  ...
  async publish(
    ...
    if (
      result &&
      result.scheduleCron &&
      result.status === AppDefStatus.ACTIVE
    ) {
      await this.schedulerService.scheduleWorkflow(
        result.id,
        result.scheduleCron,
        { applicationDefinitionId: result.id, triggeredBy: 'schedule' },
      );
    }
  ...
  async getVersions(id: string) {
    ...
    return Promise.all(
      versions.map(async (v) => {
        ...
        if (v.publishedBy) {
          publishedByInfo = await this.usersService.getUserSnapshotByUsername(
            v.publishedBy,
          );
        }
        return {
