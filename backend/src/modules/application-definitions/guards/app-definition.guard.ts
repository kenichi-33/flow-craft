...
export class AppDefinitionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (
      appDef.adminIds?.includes(user.userId) ||
      appDef.adminIds?.includes(user.username) ||
      appDef.createdBy === user.username
    ) {
      return true;
    }

    throw new ForbiddenException(
      'You do not have permission to modify this application definition',
    );
  }
}
